...
1. What is React JS and what problems does it solve? Support your answer with concepts introduced in class and from your personal research on the web.
  ...
  Ans) React JS is used primarily to facilitate the creation of single-page applications; due to the way React JS manages states, the resulting SPA can change data without needing to reload the webpage. It also allows us to break our App down into compartmentalized and reusable components, all while combining HTML, CSS, and JS into said components.

1. Describe component state.

  Ans) A compenent state represents a slice of data that can change (usually by user interaction). When a state is created, it must be given some sort of initial value (or set to null, if needed).

1. Describe props.

  Ans) Props represent the information that is passed into a given component state by its parent.

1. What are side effects, and how do you sync effects in a React component to changes of certain state or props?

  Ans) Side effects are bits of asynchronous code that run when they are called. These are useful for handling data requests from API's, for example.
